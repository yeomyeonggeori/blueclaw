...
func NewRouter(routerDependencies RouterDependencies) http.Handler {
	...
	multiplexer.HandleFunc("POST /admin/api/task/approve", routerDependencies.TaskApprovalHandler.HandleApproveTaskRun)
	...
	multiplexer.HandleFunc("POST /admin/api/task-schedules/update", routerDependencies.TaskScheduleHandler.HandleUpdate)
