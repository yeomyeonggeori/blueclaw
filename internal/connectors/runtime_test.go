...
func TestConnectorRuntimeFindsPriorTaskContextForFailedArtifactGoal(t *testing.T) {
	...
	taskRunService.AppendTaskEvent(taskRun.TaskRunID, agentcontract.TaskEventAgentIntake, string(intakeEvent))
	if _, errorValue := taskRunService.FailTaskRun(taskRun.TaskRunID, "file attach failed"); errorValue != nil {
		t.Fatal(errorValue)
	}
	event := testInboundEvent("message-deliver")
...
func TestConnectorRuntimeSendsAskUserNoticeForTargetUser(t *testing.T) {
	connectorRuntime, _, _ := newStubbedTestConnectorRuntime(t)
	connectorRuntime.taskRunService.AppendTaskEvent("task-1", agentcontract.TaskEventAskRequested, `{"kind":"input","question":"제목은 어떻게 할까요?"}`)
	sentReplies := []OutboundReply{}
	event := testInboundEvent("message-1")
...
func TestConnectorRuntimeCreatesScheduledTaskFromNaturalLanguagePrompt(t *testing.T) {
	...
	capabilityRequestBodies := []string{}
	connectorRuntime.UseTestCapabilityTools(capability.Client{
		...
		HTTPClient: testHTTPDoer(func(request *http.Request) (*http.Response, error) {
			...
			capabilityRequestBodies = append(capabilityRequestBodies, string(requestBody))
			return &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(strings.NewReader(`{"provider":"capabilityd","selectedBackend":"device","toolName":"schedule_create","outcome":"succeeded","status":"ok","content":"schedule created","result":{"scheduleID":"schedule-1","nextRunAt":"2026-06-13T07:00:00+09:00"}}`)),
	...
	if len(capabilityRequestBodies) != 1 {
		t.Fatalf("expected one schedule capability call, got %+v", capabilityRequestBodies)
	}
	for _, fragment := range []string{`"업계 뉴스를 조사해서 핵심만 보고해줘."`, `"0 7 * * *"`, `"Asia/Seoul"`, `"conversationID":"direct-1"`} {
		if !strings.Contains(capabilityRequestBodies[0], fragment) {
			t.Fatalf("expected the schedule capability call to carry %s, got %s", fragment, capabilityRequestBodies[0])
		}
	}
...
func (languageModel *recordingLanguageModel) GenerateStructuredResponse(_ context.Context, structuredResponseRequest llm.StructuredResponseRequest) (llm.StructuredResponse, error) {
	...
	if structuredResponseRequest.StructuredOutputSchema.Name == "bluecollar_expected_changes" {
		return llm.StructuredResponse{Content: `{"expectedChanges":[]}`}, nil
	}
...
func appendConnectorActiveGoal(t *testing.T, taskRunService *task.TaskRunService, taskRun task.TaskRun, activeGoal agentcontract.ActiveGoal) {
	...
	taskRunService.AppendTaskEvent(taskRun.TaskRunID, agentcontract.TaskEventAgentGoalBlocked, string(document))
}
...
func TestANewRequestWhileAConfirmationIsPendingLeavesItPendingAndIsRoutedWithTheCatalog(t *testing.T) {
	invokedTools := []string{}
	languageModel := agenttest.NewScriptedLanguageModel(agenttest.ScriptedLanguageModelOptions{
		ChatResponsesBySchema: map[string][]string{
	...
	thirdEvent.Prompt = "ㅇ"
	thirdResult, errorValue := connectorRuntime.HandleInboundEvent(context.Background(), adapter, thirdEvent)
	if errorValue != nil {
		t.Fatalf("expected the approval to process: %v", errorValue)
	}
	if thirdResult.TaskRunID != firstResult.TaskRunID {
		t.Fatalf("the approval that follows belongs to the confirmation that waited, got %+v", thirdResult)
	}
	if len(invokedTools) != 1 || invokedTools[0] != "event_delete/invoke" {
		t.Fatalf("expected the approved delete to run once, got %+v", invokedTools)
	}
}

func TestAQuestionAboutThePendingConfirmationLeavesItPending(t *testing.T) {
	languageModel := agenttest.NewScriptedLanguageModel(agenttest.ScriptedLanguageModelOptions{
		ChatResponsesBySchema: map[string][]string{
	...
	firstTaskRun, _ := connectorRuntime.taskRunService.FindTaskRun(firstResult.TaskRunID)
	if firstTaskRun.Status != task.TaskStatusWaitingApproval {
		t.Fatalf("answering a question about the confirmation does not withdraw it, got %s", firstTaskRun.Status)
	}
