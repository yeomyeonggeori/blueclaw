...
func TestBuildTimelineMarksFailedToolResult(testInstance *testing.T) {
	taskEvents := []TaskEvent{
		{Name: "tool.bash.requested", Body: `{"toolName":"bash"}`, CreatedAt: time.Unix(0, 0)},
		{Name: "tool.bash.result", Body: `{"failure":{"userSafeSummary":"permission denied","code":"operation_failed"}}`, CreatedAt: time.Unix(1, 0)},
	}
