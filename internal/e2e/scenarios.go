...
func PlatformMessageEditAcceptanceScenario(artifactDirectoryPath string) VirtualSessionScenario {
	return VirtualSessionScenario{
		...
		AllowedTools:           append(toolcontract.KernelToolNames(), "message_update"),
		CapabilityToolNames:    []string{"message_update"},
		InitialToolNames:       []string{"message_update"},
		Turns: []VirtualTurn{{
			...
			RouterApproval: "approve",
			ActionResponses: []string{
				actionFinishMessage("공지 메시지 문구를 수정했습니다.", "obs-001:message_update:0"),
			},
			CompletionJudgeResponses: []string{completionJudgeSatisfiedResponse()},
			ExpectedToolCalls:        []string{"message_update"},
			ExpectedToolCallCounts: map[string]int{
				"message_update": 1,
			},
			ExpectedEventCounts: []VirtualEventCount{
