...
func mixedToolSet(t *testing.T) *toolcontract.ToolSet {
	t.Helper()
	toolNames := []string{toolcontract.BashToolName, toolcontract.FileReadToolName, toolcontract.EquipToolName, "event_add", toolcontract.AskConfirmToolName}
...
func TestASelfEquippedHarnessIsNotHandedTheToolsItAlreadyHas(t *testing.T) {
	...
	for _, harnessOwned := range []string{toolcontract.BashToolName, toolcontract.FileReadToolName, toolcontract.EquipToolName} {
...
func TestABareHarnessStillGetsEverything(t *testing.T) {
	...
	for _, toolName := range []string{toolcontract.BashToolName, toolcontract.FileReadToolName, toolcontract.EquipToolName, "event_add", toolcontract.AskConfirmToolName} {
