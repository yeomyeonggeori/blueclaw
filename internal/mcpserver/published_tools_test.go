...
func mixedToolSet(t *testing.T) *toolcontract.ToolSet {
	t.Helper()
	toolNames := []string{toolcontract.ShellToolName, toolcontract.FileReadToolName, toolcontract.FindToolsToolName, "event_add", toolcontract.AskConfirmToolName}
...
func TestASelfEquippedHarnessIsNotHandedTheToolsItAlreadyHas(t *testing.T) {
	...
	for _, harnessOwned := range []string{toolcontract.ShellToolName, toolcontract.FileReadToolName, toolcontract.FindToolsToolName} {
...
func TestABareHarnessStillGetsEverything(t *testing.T) {
	...
	for _, toolName := range []string{toolcontract.ShellToolName, toolcontract.FileReadToolName, toolcontract.FindToolsToolName, "event_add", toolcontract.AskConfirmToolName} {
