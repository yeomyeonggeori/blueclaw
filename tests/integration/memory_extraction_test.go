...
func (reader *integrationTaskRunReader) FindTaskRun(taskRunID string) (agentcontract.TaskRun, bool) {
	taskRun, isFound := reader.taskRuns[taskRunID]
	return taskRun, isFound
}

func (reader *integrationTaskRunReader) ListTaskEvent(taskRunID string) []agentcontract.TaskEvent {
...
func (reader *integrationTaskRunReader) AppendTaskEvent(taskRunID string, name string, body string) {
	reader.events[taskRunID] = append(reader.events[taskRunID], agentcontract.TaskEvent{TaskRunID: taskRunID, Name: name, Body: body})
}
...
func TestMemoryExtractionRecordsFactsAndProfileInPostgres(t *testing.T) {
	...
	reader := &integrationTaskRunReader{
		taskRuns: map[string]agentcontract.TaskRun{"run-1": {
			TaskRunID:         "run-1",
		...
		events: map[string][]agentcontract.TaskEvent{"run-1": {{Name: memory.ExtractionContextEventName, Body: `{"requesterName":"이샘플","securityLevelRank":1,"requiredClasses":[]}`}}},
	}
