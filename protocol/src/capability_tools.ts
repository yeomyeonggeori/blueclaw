...
const fileToolDefinitions: CapabilityToolDefinition[] = [
  {
    ...
    description: 'Read a workspace document and return Markdown content. path takes an exact /workspace path, a fileHint from the attachment catalog, or the exact file URL as it appears in a message text — a document attached to an earlier message is fetched by that URL. Use image_read for image files.',
    version: '1',
    estimatedLatency: CapabilityEstimatedLatency.High,
  ...
  },
  {
    ...
    description: 'Read a workspace image and return a base64 attachment. path takes an exact /workspace path, a fileHint from the attachment catalog, or the exact media URL as it appears in a message text — an image attached to an earlier message is fetched by that URL, so never invent a filesystem path from a URL. Use document_read for document files.',
    version: '1',
    estimatedLatency: CapabilityEstimatedLatency.Medium,
